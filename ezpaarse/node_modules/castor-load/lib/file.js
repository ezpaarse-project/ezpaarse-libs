...
function File(pathname, stats, startobj)
{
  ...
  self.doc.openStream = function(opts) {
    ...
    if (this.filetype === 'url') {
      extend(self.requestOptions, opts)
      self.requestOptions.parse = false;
      return require('needle').get(this.location, self.requestOptions);
    }
    else {
...
File.prototype.analyze = function (fn) {
  ...
  if (self.doc.sha1 === undefined) {
    if (self.doc.filetype === 'file') {
      ...
      request.head(self.doc.location, self.requestOptions, function(error, response) {
        if (error) {
          fn(error, self.doc);
        }
